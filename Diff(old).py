...
def compareData(originalData: Any, newData: Any, tolerance: float = 0.0) -> Dict[str, Any]:
    ...
    def _compare(value1: Any, value2: Any, path: str = '') -> List[Dict]:
        ...
        # ---------- 新增：标准化容器类型与扁平化降维容错 ----------
        # 1. 统一元组和 numpy 数组为列表，消除基本的 tuple/list/ndarray 类型隔阂
        if isinstance(value1, (tuple, np.ndarray)):
            value1 = value1.tolist() if isinstance(value1, np.ndarray) else list(value1)
        if isinstance(value2, (tuple, np.ndarray)):
            value2 = value2.tolist() if isinstance(value2, np.ndarray) else list(value2)

        # 2. 消除因 Matlab 转 Python 常见的多余单维度问题 (例如 [1, 2] vs [[1], [2]] 或 [[1, 2]])
        if isinstance(value1, list) and isinstance(value2, list):
            try:
                # 使用 object 类型避免深度解析内部 dict 时失败
                arr1 = np.array(value1, dtype=object)
                arr2 = np.array(value2, dtype=object)
                if arr1.shape != arr2.shape:
                    sq1 = np.squeeze(arr1)
                    sq2 = np.squeeze(arr2)
                    
                    # 尝试 1：如果压缩掉长度为 1 的维度后两边形状一致，直接采用解压后结果
                    if sq1.shape == sq2.shape:
                        value1 = sq1.tolist()
                        value2 = sq2.tolist()
                        # 防止 squeeze 后变回了单个对象(如 dict)，导致下方丢失 list 处理逻辑，补回包层
                        if not isinstance(value1, list): value1 = [value1]
                        if not isinstance(value2, list): value2 = [value2]
                        
                    # 尝试 2 (创意兼容)：如果形状依旧不同，但两边包含的元素总数、且总大小大于 0，说明存在更深的不规则维度错位
                    elif sq1.size == sq2.size and sq1.size > 0:
                        value1 = sq1.flatten().tolist()
                        value2 = sq2.flatten().tolist()
            except Exception:
                # 如果 np.array 处理变长列表遇到报错，则静默吃掉继续尝试用普通的 list 遍历报错
                pass
        # ------------------------------------------------
